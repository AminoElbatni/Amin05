...
def gmail():
		...
		with tred(max_workers=30) as AMIN:
			...
			for user in fo:
				...
				if mthd in ['1','01']:
					AMIN.submit(trt1,ids,passlist)
				if mthd in ['2','02']:
					AMIN.submit(trt2,ids,passlist)
				if mthd in ['3','03']:
					AMIN.submit(trt3,ids,passlist)
				if mthd in ['4','04']:
					AMIN.submit(trt4,ids,passlist)
				if mthd in ['5','05']:
					AMIN.submit(trt5,ids,passlist)
				if mthd in ['6','06']:
					AMIN.submit(trt6,ids,passlist)
		print('\033[1;37m')
		linex()
		print(' The process has completed')
		print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
